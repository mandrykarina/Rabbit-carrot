...
public:
    ...
    {
        return !(*this == other); // возвращаем результат логического отрицания оператора ==
    }
...
const int WIDTH = 140;
const int HEIGHT = 40;
const int BASE_SPEED = 180; // базовая задержка между кадрами в миллисекундах
const int MAX_ITEMS = 6; //Базовое ограничение количества падающих предметов 
const int SPEED_STEP = 20;//на сколько миллисекунд уменьшается задержка между кадрами при каждом ускорении
const int SPEED_INTERVAL = 20;//Промежуток времени в секундах, через который происходит следующее ускорение
...
private:
    ...
    int game_speed;            // Определяет задержку между кадрами
